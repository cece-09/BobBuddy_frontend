...
import { Avatar, Container, CssBaseline, Link, Typography } from '@mui/material';
...
const ProfilePage = () => {
  ...
  const [updatedProfileData, setUpdatedProfileData] = useState<User | null>(null);
      ...
      >
        ...
        <Box sx={{ width: '90%' }}>
          <Stack spacing={3}>
            ...
            <UpdateItem onClick={() => handleUpdateItemClick('birth')}>생년월일: {user.userData.birth}</UpdateItem>
            <UpdateItem onClick={() => handleUpdateItemClick('gender')}>성별: {user.userData.gender}</UpdateItem>
            <UpdateItem onClick={() => handleUpdateItemClick('mbti')}>MBTI: {user.userData.mbti}</UpdateItem>
            <UpdateItem onClick={() => handleUpdateItemClick('favoriteFood')}>좋아하는 음식: {user.userData.favoriteFood}</UpdateItem>
            <UpdateItem onClick={() => handleUpdateItemClick('dislikedFood')}>싫어하는 음식: {user.userData.dislikedFood}</UpdateItem>
          </Stack>
        </Box>
        <Box
          sx={{
